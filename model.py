...
class DocumentAnalysisAgent:
    def __init__(self, hf_api_key: str):
        ...
        self.model_name = "meta-llama/Llama-3.1-8B-Instruct"  # You can choose a different model if desired
        self.client = InferenceClient(api_key=self.api_key, model=self.model_name)
    ...
    def analyze_with_llama(self, query: str, context: str = None) -> str:
        """Send query to Llama model for analysis via Hugging Face API"""
...
def main():
    ...
    with st.sidebar:
        ...
        # API Key input from secrets
        api_key = st.secrets.get("HF_API_KEY", "")

        if api_key:
            if st.session_state.agent is None:
                st.session_state.agent = DocumentAnalysisAgent(api_key)
                st.success("Agent initialized with Hugging Face API!")
        else:
            st.error("HF_API_KEY not found in secrets. Please configure it.")
